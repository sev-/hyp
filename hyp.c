...
/*------------------------------------------------------------------------*/
  ...
  if(argc<2)
  {
    puts("Просмотрщик гипертекста. Версия 2.1\n\n\
 Использование:\n\t\thyp file\n");
    exit(1);
  }
...
/*----------------------------------------------------------------------*/
  ...
  if(( infile = fopen(name_all, "r" )) == (FILE *)NULL)
  {
...
/*-------------------------------------------------------------------------*/
  ...
  /*-----------------Открытие файла по HOME---------------------------*/
    ...
    if(strcmp(current_file, path_file) != 0 && strchr(path_file, '*') == (char *)NULL )
    {
      fclose(infile);
      sprintf(name_all, "%s/%s", dir, path_file);
      if(( infile = fopen(name_all, "rt" )) == (FILE *)NULL)
      {
        vcend(CLOSE);
	  ...
	  break;
        ...
        case GO_TREE:
          if(strchr(path_file, '*') != (char *)NULL)
            ...
            if(strcmp(current_file, path_file) != 0)
            {
              sprintf(name_all, "%s/%s", dir, path_file);
              if(( tempfile = fopen(name_all, "rt" )) == (FILE *)NULL)
              {
        ...
        case TREE_UP_KEY:
          ...
          break;
        case ESC:
          if(!ask_msg("Вы уверены что хотите выйти из ГИПЕРТЕКСТА ?.", 1))
            break;
          fclose(infile);
          wclose(wdo1);
          vcend(NOCLOSE);
