/*
 *  $Id: edit_str.c,v 1.1 1994/03/05 21:56:56 sev Exp $
 *
 * ---------------------------------------------------------- 
 *
 * $Log: edit_str.c,v $
 * Revision 1.1  1994/03/05 21:56:56  sev
 * Initial revision
 *
 * Revision 1.1  1993/03/04  09:50:43  kas
 * Initial revision
 *
 *
*/

static char rcsid[]="$Id: edit_str.c,v 1.1 1994/03/05 21:56:56 sev Exp $";
...
/* Функция позволяет редактировать в окне заданного размера
   строку неограниченной длины, сдвигая ее влево иои вправо
   и помечая выход строки за рамки окна. Функция возвращает
   отредактированную строку в том же буфере, в котором была
   передана.
   Вход:
	row             - номер строки окна на экране;
	start_col       - левая координата окна на экране;
	end_col         - правая координата окна на экране;
	buffersize      - размер бувера строки, байт;
	originalstring  - указатель на редактируемую строку;
	title           - указатель на заголовок окна;
	sourceattr      - атрибуты для нередактированной строки;
	destattr        - атрибуты редактированной строки;
   Выход:
	Функция возвращает 0, если строка не редактировалась
	( была нажата  клавиша ESCAPE ), либо 1, если строка
...
char  destattr;
{
  ...
  /* скопировать строку в буфер и получить ее длину, задать начальное
			     значение остатка */
