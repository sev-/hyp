/*
 *  $Id: edhmark.c,v 1.3 1993/04/20 16:04:12 sev Exp $
 *
 * ---------------------------------------------------------------------------
 *
 * $Log: edhmark.c,v $
...
/*              vcedstatus(vced);               */
		...
		case POLE:
			...
			if(flag_str)
			{
				...
				wxatsay(w,0,3,"Oтметка поля       ",ATR_F);
				selbar(w,0,0,vc.white*vc.bg,79);
				edprtline(vced,vced->edcline,from1,to1,stat_);
				col_str=1;
				break;
			}
...
/*
			wxatsay(w,0,3,"Поле отмечено      ",ATR_F);
			selbar(w,0,0,vc.white*vc.bg,79);
*/
		...
		case DEL_POLE:
			...
			edprtline(vced,vced->edcline,from1,to1,stat_);
/*
			wxatsay(w,0,3,"Отмена поля      ",ATR_F);
			selbar(w,0,0,vc.white*vc.bg,79);
*/
			vced->edupval |= VCEDUPDLINE;
			vced->edsline = 1;
