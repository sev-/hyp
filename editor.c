...
{                                               /* ------------------------ */
	...
	dirr    = aloselitm();   /* init list of segments                   */
	...
	vcterm.cols-1,NULLTEXT,ACTIVE,0,0,1,32);      /* Window             */
...
{                                   /* ------------------------------------ */
	...
	77,NULL,ACTIVE|SCROLL|CURSOR,0,0,1,32);
	atsay(0,1,"Имя файла :                       ");
...
struct VCEDFILE   *tmpfile1;
{
	...
	curwdo->vcedwvced->edupval = VCEDUPDALL;       /* Update            */
